...
const MurmurHash3 = tslib_1.__importStar(require("imurmurhash"));
class Compiler {
    _userName;
    _gameName;
    ...
    _compiledSource = "";
    _urlFE = "";
    _azStorage = new AzStorage_1.AzStorage();
    _tsc;
    constructor(_userName, _gameName) {
    ...
    async _npm() {
        await new Promise(resolve => {
            (0, child_process_1.exec)("npm i", { cwd: `/active/user_${this._userName}_${this._gameName}/` }, async (error, stdout, stderr) => {
                if (error) {
    ...
    async _spawntsc() {
        this._tsc = (0, child_process_1.spawn)("npx", ["frakas", "build"], { cwd: `/active/user_${this._userName}_${this._gameName}/` });
        console.log(`spawn("npx", ["frakas", "build" ], { cwd: /active/user_${this._userName}_${this._gameName}/ });`);
        this._tsc.stdout.on("data", async (data) => {
